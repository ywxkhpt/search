...
def get_data(the_host='mongodb://127.0.0.1:27017',
    ...
    with codecs.open("linkedin_data.txt", 'a', encoding='utf-8') as f:
        for user in user_detail:
            ...
            if user["person_website"] is None:
                ...
                f.write(person_website)
                f.write("\n")
            if user["company_location"] is None:
                company_location = "company_location:"
                f.write(company_location)
                f.write("\n")
            else:
                company_location = "company_location:" + user["company_location"]
                f.write(company_location)
                f.write("\n")
            if user["background_summary"] is None:
                background_summary = "background_summary:"
                f.write(background_summary)
                f.write("\n")
            else:
                background_summary = "background_summary:" + user["background_summary"]
                f.write(background_summary)
                f.write("\n")
